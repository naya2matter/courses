...
function StatTile({
  ...
  return (
    <div className="rounded-xl border border-white/8 bg-white/3 p-3">
      ...
      <div className="mt-1.5 text-sm font-medium text-foreground wrap-break-word">{children}</div>
    </div>
  )
}
...
function ModuleSection({ module: mod }: { module: OnlineCourseModule }) {
  ...
  return (
    <details className="group rounded-2xl border border-white/8 bg-white/2 overflow-hidden" open>
      {/* Summary */}
        ...
        <>
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4">
              <StatTile icon={<CircleDotIcon className="h-3.5 w-3.5" />} label="Status">
                <Badge className={`text-[11px] border ${statusBadgeClass(course.status)}`}>{course.status}</Badge>
              </StatTile>

              {course.level && (
                <StatTile icon={<GraduationCapIcon className="h-3.5 w-3.5" />} label="Level">
                  {levelLabel(course.level)}
                </StatTile>
              )}

              {course.estimated_duration && (
                <StatTile icon={<ClockIcon className="h-3.5 w-3.5" />} label="Est. Duration">
                  {formatDuration(course.estimated_duration * 60)}
                </StatTile>
              )}

              <StatTile icon={<LayersIcon className="h-3.5 w-3.5" />} label="Modules">
                {moduleCount} module{moduleCount !== 1 ? "s" : ""}
                {totalContents > 0 && ` · ${totalContents} item${totalContents !== 1 ? "s" : ""}`}
              </StatTile>

              {course.creator && (
                <StatTile icon={<UserIcon className="h-3.5 w-3.5" />} label="Created By">
                  {course.creator.name}
                </StatTile>
              )}
              ...
              {course.deadline && (
                <StatTile icon={<CalendarClockIcon className="h-3.5 w-3.5" />} label="Deadline">
                  <span className="text-amber-400">{formatDate(course.deadline)}</span>
                </StatTile>
              )}
            </div>
          </section>
          ...
          {course.description && (
            <section className="space-y-3">
              ...
              <div className="rounded-2xl border border-white/8 bg-white/2 p-5 text-sm text-foreground/80 whitespace-pre-wrap leading-relaxed">
                {course.description}
              </div>
            </section>
          )}
