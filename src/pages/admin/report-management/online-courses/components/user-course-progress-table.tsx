...
const COLS = [
  { header: "User" }, { header: "Course" }, { header: "Progress", align: "right" as const },
  { header: "Status" }, { header: "Compliance" }, { header: "Score Band" },
  { header: "Overdue", align: "right" as const },
]
...
export function UserCourseProgressTable({ data, meta, isLoading, error, page, onPageChange, onRetry }: Props) {
  const rows = data.map((r, i) => (
    <TableRow key={`${r.user_id}-${r.course_online_id}-${i}`} className="border-white/5 hover:bg-white/3">
      <TableCell>
      ...
      <TableCell className="text-right">
        <div className="h-1.5 w-20 overflow-hidden rounded-full bg-white/10 ml-auto">
          <div className={`h-full rounded-full ${r.status === "completed" ? "bg-emerald-500" : "bg-indigo-500"}`} style={{ width: `${r.progress_percentage}%` }} />
        </div>
        <span className="text-xs text-white/50 tabular-nums">{Number(r.progress_percentage).toFixed(0)}%</span>
      </TableCell>
      <TableCell>
        <Badge variant="outline" className={`rounded-full text-[10px] capitalize ${STATUS_CFG[r.status] ?? ""}`}>
          {r.status.replace("_", " ")}
        </Badge>
      </TableCell>
      <TableCell>
      ...
      <TableCell>
        <Badge variant="outline" className={`rounded-full text-[10px] capitalize ${SCORE_CFG[r.score_band] ?? ""}`}>
          {r.score_band}
        </Badge>
      </TableCell>
      <TableCell className="text-right text-sm">
        {r.days_overdue > 0
          ? <span className="font-semibold text-rose-400">{r.days_overdue}d</span>
