...
import type { CourseRegistration, PaginationMeta } from "../types/live-report.types"
...
export function RegistrationsTable({ data, meta, isLoading, error, page, onPageChange, onRetry }: Props) {
  if (error) {
    return (
        ...
        <Table className="text-sm">
          ...
          <TableBody>
            ...
            {!isLoading && data.length > 0 && data.map((r) => (
              <TableRow key={r.registration_id} className="border-white/5 hover:bg-white/3">
                <TableCell>
                ...
                <TableCell>
                  <Badge
                    variant="outline"
                    className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold capitalize ${STATUS_CFG[r.status] ?? ""}`}
                  >
                    {r.status.replace("_", " ")}
                  </Badge>
                </TableCell>
      ...
      {meta && meta.last_page > 1 && (
        <div className="flex items-center justify-between text-sm text-white/40">
          ...
          <div className="flex gap-2">
            ...
            <Button
              variant="outline" size="sm" disabled={page >= meta.last_page}
              onClick={() => onPageChange(page + 1)}
              className="border-white/10 bg-white/5 text-white/60 hover:bg-white/10"
            >
              Next
