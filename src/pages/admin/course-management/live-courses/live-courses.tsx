...
/**
 * Display course capacity bar based on availabilities
 */
function CourseCapacityBar({ availabilities }: { availabilities: unknown }) {
  const { parsed, totalCapacity, totalUsed, sessionsCount, startLabel } =
    getAvailabilityMetrics(availabilities)
  if (!parsed.length) return null

  if (totalCapacity === 0) {
    return (
      <div className="flex items-center gap-1.5 text-sm text-white/80 mt-3 font-medium">
        <CalendarIcon className="h-4 w-4" />
        <span>
          Starts {startLabel} • {totalCapacity > 0 ? `${totalCapacity} seats` : "No seats defined"} • {sessionsCount} sessions
        </span>
      </div>
    )
  }

  const percentage = Math.min(100, Math.max(0, (totalUsed / totalCapacity) * 100))
  ...
  return (
    <div className="space-y-2 mt-4 pt-1">
      <div className="flex items-center justify-between text-xs font-medium text-white/90">
        ...
        <span className="text-white/80">{Math.round(percentage)}% filled</span>
      </div>
      <div className="h-2 w-full bg-white/20 rounded-full overflow-hidden ring-1 ring-black/10">
        <div 
          className="h-full bg-gradient-to-r from-indigo-400 to-purple-400 rounded-full transition-all duration-500 ease-out" 
          style={{ width: `${percentage}%` }}
        />
      </div>
      <p className="text-[10px] text-white/60 text-right mt-1.5">{totalUsed} of {totalCapacity} seats ({sessionsCount} sessions)</p>
    </div>
  )
}
...
export default function LiveCoursesPage() {
  ...
  function handleCourseClick(id: number) {
    navigate(`/admin/course-management/live-courses/${id}`)
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {items.map((course) => (
            ...
            >
              ...
              {/* Background styling for professional dark card look */}
            <div className="absolute inset-0 z-0 bg-zinc-900 overflow-hidden">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/80 to-zinc-900/40 z-10" />
              </div>

              <div className="relative z-10 p-5 flex flex-col h-full min-h-[16rem] justify-between">
                {/* Header: Badges & Actions */}
                <div className="flex items-start justify-between gap-2 mb-6">
                  <Badge variant={getStatusVariant(course.status)} className="backdrop-blur-md bg-background/50 border-white/10">
                    {course.status || "Unknown"}
                  </Badge>
                  <div className="flex items-center gap-1 backdrop-blur-md bg-background/30 rounded-md p-0.5 border border-white/10">
                    {course.privacy === "private" ? (
                      <LockIcon className="h-4 w-4 text-white/70 mx-1" />
                    ) : (
                      <UnlockIcon className="h-4 w-4 text-white/70 mx-1" />
                    )}
                    {course.privacy === "private" && (
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-7 w-7 text-white/70 hover:text-emerald-400 hover:bg-emerald-400/20 rounded-sm"
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 text-white/70 hover:text-white hover:bg-white/20 rounded-sm"
                      onClick={(e) => {
                        e.stopPropagation()
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 text-red-400 hover:text-red-300 hover:bg-red-400/20 rounded-sm"
                      onClick={(e) => {
                        e.stopPropagation()
                ...
                <div className="space-y-4">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <h3 className="text-2xl font-semibold text-white tracking-tight leading-tight line-clamp-2">
                        {course.name}
                      </h3>
                      {course.level && (
                    ...
                    <div className="flex items-center gap-2 bg-white/10 text-white/80 rounded-full px-3 py-1.5 text-sm">
                  ...
                  {course.availabilities && (
                    <CourseCapacityBar availabilities={course.availabilities} />
                  )}

                  {course.availabilities && (
                    <CourseAvailabilitiesCollapse availabilities={course.availabilities} dark />
                  )}
                </div>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          <div className="mt-2 flex flex-wrap gap-2 text-sm text-muted-foreground">
                            ...
                            <span className="rounded-full bg-muted/20 px-3 py-1">ID {course.id}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          {course.privacy === "private" && (
