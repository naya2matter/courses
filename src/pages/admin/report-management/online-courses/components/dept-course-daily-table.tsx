...
const COLS = [
  { header: "Date" }, { header: "Department" }, { header: "Course" },
  { header: "Active Users", align: "right" as const }, { header: "Sessions", align: "right" as const },
  { header: "Completions", align: "right" as const }, { header: "Watch Time", align: "right" as const },
  { header: "Avg Attention", align: "right" as const },
]
...
export function DeptCourseDailyTable({ data, meta, isLoading, error, page, onPageChange, onRetry }: Props) {
  const rows = data.map((r, i) => (
    <TableRow key={r.id ?? i} className="border-white/5 hover:bg-white/3">
      ...
      <TableCell className="text-right text-white/70">{r.active_users}</TableCell>
